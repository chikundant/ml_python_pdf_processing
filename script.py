...
class SimpleMLEngine:
    ...
    def find_relevant_chunks(
        self, query: str, chunks_data: List[Dict], top_k: int = 5
        ...
        try:
            ...
            chunk_texts = [
                self.preprocess_text(chunk["chunk_text"]) for chunk in chunks_data
            ]
            ...
            for idx in top_indices:
                if (
                    idx < len(chunks_data) and similarities[idx] > 0.05
                ):  # Relevance threshold
    ...
    def generate_answer(
        self, query: str, relevant_chunks: List[Tuple[Dict, float]]
    ) -> Dict:
...
async def ask_question(request: QuestionRequest):
    ...
    try:
        ...
        relevant_chunks = ml_engine.find_relevant_chunks(
            request.question, chunks_data, top_k=5
        )
        ...
        logger.info(
            f"Question: {request.question}, Confidence: {response['confidence']:.3f}"
        )
